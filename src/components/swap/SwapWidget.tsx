...
import { Settings, ArrowUpDown, Zap, Info, RefreshCw, AlertTriangle } from "lucide-react";
...
export default function SwapWidget() {
  ...
  const { data: allowance, refetch: refetchAllowance } = useReadContract({
    ...
    args: [address!, CONTRACTS.UNIVERSAL_ROUTER as `0x${string}`],
  ...
  // ── Quote ────────────────────────────────────────────────────
  const parsedAmountIn = tokenIn && amountIn
  ...
  const [bestAmountOut, bestPool] = quoteData
    ? [quoteData[0] as bigint, quoteData[1] as number]
    : [0n, 0];

  const amountOutFormatted = tokenOut && bestAmountOut > 0n
    ? parseFloat(formatUnits(bestAmountOut, tokenOut.decimals)).toFixed(6)
    : "";

  const priceImpact = bestAmountOut > 0n && parsedAmountIn > 0n ? 0.12 : 0; // simplified
  ...
  function handleSwap() {
    if (!tokenIn || !tokenOut || !address || parsedAmountIn === 0n || bestAmountOut === 0n) return;
  ...
  const balInFormatted = balanceIn && tokenIn
    ? parseFloat(formatUnits(balanceIn as bigint, tokenIn.decimals)).toFixed(4)
    : "0.0000";
  const balOutFormatted = balanceOut && tokenOut
    ? parseFloat(formatUnits(balanceOut as bigint, tokenOut.decimals)).toFixed(4)
    : "0.0000";
    ...
    <div className="w-full max-w-[460px] mx-auto">
      ...
      <div className="glass-card rounded-3xl p-4 shadow-[0_8px_48px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className={clsx(
                ...
                showSettings
                  ? "text-[#00c8e8] bg-[rgba(0,200,232,0.1)]"
        ...
        {showSettings && (
          <div className="mb-4 p-3 rounded-2xl bg-[rgba(0,0,0,0.3)] border border-[rgba(0,200,232,0.08)] animate-slide-up">
            <div className="text-xs font-semibold text-white/50 mb-2.5"
        ...
        <div className="rounded-2xl bg-[rgba(0,0,0,0.35)] border border-[rgba(255,255,255,0.05)] p-3 mb-1">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {balanceIn && (balanceIn as bigint) > 0n && (
                <button
                  onClick={() => tokenIn && setAmountIn(
        ...
        <div className="rounded-2xl bg-[rgba(0,0,0,0.35)] border border-[rgba(255,255,255,0.05)] p-3 mt-1 mb-3">
          ...
          <div className="flex items-center gap-3">
            <div className={clsx("flex-1 text-2xl font-mono", quoting && parsedAmountIn > 0n ? "text-white/30" : "text-white")}>
              {quoting && parsedAmountIn > 0n ? (
                <span className="shimmer text-transparent bg-clip-text">Loading...</span>
              ) : (
        ...
        {bestAmountOut > 0n && (
          <div className="mb-3 px-3 py-2.5 rounded-xl bg-[rgba(0,200,232,0.04)] border border-[rgba(0,200,232,0.08)] text-xs space-y-1.5">
            ...
            <div className="flex justify-between text-white/50">
              ...
              <span className={priceImpact > 3 ? "text-red-400" : "text-green-400"}>
            ...
            <div className="flex justify-between text-white/50">
              ...
              <span className="font-mono text-white/70">
                {tokenOut && bestAmountOut > 0n
                  ? parseFloat(formatUnits(bestAmountOut * BigInt(10000 - Math.floor(slippage * 100)) / 10000n, tokenOut.decimals)).toFixed(6)
                  : "—"
                } {tokenOut?.symbol}
              </span>
            </div>
        ...
        {wrongNetwork ? (
          ...
          <button
            onClick={handleSwap}
            disabled={btnLoading || !parsedAmountIn || parsedAmountIn === 0n || !bestAmountOut}
      ...
      {swapSuccess && swapTxHash && (
        <div className="mt-3 p-3 rounded-xl bg-[rgba(0,200,100,0.08)] border border-[rgba(0,200,100,0.2)] text-sm text-green-400 text-center animate-fade-in">
          ...
          <a
            href={`https://monadscan.com/tx/${swapTxHash}`}
            target="_blank"
