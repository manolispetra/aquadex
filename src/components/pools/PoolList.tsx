...
function PoolSkeleton() {
  return (
    <div className="pool-row px-5 py-4 grid grid-cols-4 gap-4">
      {[1,2,3,4].map(i => <div key={i} className="shimmer h-4 rounded" />)}
    </div>
  );
}
...
export default function PoolList() {
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden">
      ...
      {!pairAddresses && count > 0 && [1,2,3].map(i => <PoolSkeleton key={i} />)}
      ...
      {pairAddresses?.map((result, i) => {
        ...
        return <PoolRow key={pairAddr} address={pairAddr} />;
      })}
...
function PoolRow({ address }: { address: string }) {
  const { data: token0 } = useReadContract({ address: address as `0x${string}`, abi: V2_PAIR_ABI, functionName: "token0" });
  const { data: token1 } = useReadContract({ address: address as `0x${string}`, abi: V2_PAIR_ABI, functionName: "token1" });
  const { data: reserves } = useReadContract({ address: address as `0x${string}`, abi: V2_PAIR_ABI, functionName: "getReserves" });
  const { data: sym0 } = useReadContract({ address: token0 as `0x${string}`, abi: ERC20_ABI, functionName: "symbol", query: { enabled: !!token0 } });
  const { data: sym1 } = useReadContract({ address: token1 as `0x${string}`, abi: ERC20_ABI, functionName: "symbol", query: { enabled: !!token1 } });
    ...
    <div className="pool-row px-5 py-4 grid grid-cols-4 gap-4 items-center group">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-white/30 font-mono">{address.slice(0,6)}...{address.slice(-4)}</div>
        </div>
      </div>
      <div className="text-sm font-mono text-white/70">{parseFloat(tvl) > 0 ? "~" + parseFloat(tvl).toFixed(2) + " LP" : "—"}</div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden group-hover:flex items-center gap-2">
          <Link href={"/pools/add?token0=" + token0 + "&token1=" + token1}
